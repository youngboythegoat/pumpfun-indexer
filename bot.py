...
def get_user_subscriptions(user_id: int):
    ...
    try:
        ...
        print(f"DB Error in get_user_subscriptions: {e}")
        return []
    finally:
        cur.close()
        conn.close()
...
def remove_subscription(user_id: int, tweet_id: str):
    ...
    try:
        ...
        print(f"DB Error in remove_subscription: {e}")
    finally:
        cur.close()
        conn.close()
...
def search_coins_by_tweet(tweet_id: str):
    ...
    try:
        ...
        print(f"DB Error in search_coins_by_tweet: {e}")
        return []
    finally:
        cur.close()
        conn.close()
...
def get_recent_coins(minutes: int = 10):
    ...
    try:
        ...
        print(f"DB Error in get_recent_coins: {e}")
        return []
    finally:
        cur.close()
        conn.close()
...
def get_subscribers_for_tweet(tweet_id: str):
    ...
    try:
        ...
        print(f"DB Error in get_subscribers_for_tweet: {e}")
        return []
    finally:
        cur.close()
        conn.close()
...
def has_been_notified(user_id: int, mint: str):
    ...
    try:
        ...
        print(f"DB Error in has_been_notified: {e}")
...
def record_notification(user_id: int, mint: str):
    ...
    try:
        ...
        print(f"DB Error in record_notification: {e}")
    finally:
        cur.close()
        conn.close()
...
async def find(interaction: discord.Interaction, tweet: str):
    try:
        ...
        if not tweet_id:
            await interaction.followup.send("Invalid tweet URL or ID. Please use a proper link or numeric ID.", ephemeral=True)
            return

        results = search_coins_by_tweet(tweet_id)
        if not results:
            await interaction.followup.send("No matching coins found for this tweet.", ephemeral=True)
            return
        ...
        await interaction.followup.send("Something went wrong while searching. Please try again later.", ephemeral=True)
...
async def notify(interaction: discord.Interaction, tweet: str, channel: discord.TextChannel = None):
    try:
        tweet_id = extract_tweet_id(tweet)
        if not tweet_id:
            await interaction.response.send_message("Invalid tweet URL or ID. Please use a proper link or numeric ID.", ephemeral=True)
            return
        ...
        await interaction.response.send_message("Failed to set up notification. Please try again.", ephemeral=True)
...
async def mynotifications(interaction: discord.Interaction):
    try:
        ...
        print(f"Error in /mynotifications: {e}")
        await interaction.response.send_message("Failed to fetch your notifications.", ephemeral=True)
...
async def stopnotify(interaction: discord.Interaction, tweet: str):
    try:
        ...
        print(f"Error in /stopnotify: {e}")
        await interaction.response.send_message("Failed to remove notification.", ephemeral=True)
